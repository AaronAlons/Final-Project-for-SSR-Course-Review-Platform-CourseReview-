<nav x-data="{ open: false }" class="bg-white border-b border-gray-100">
    ...
    <a href="{{ Auth::check() ? route('dashboard') : route('home') }}">
        ...
        />
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    @auth
                        {{-- Enlace principal al Dashboard solo si está autenticado --}}
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                @guest
                    {{-- Si es invitado, mostrar enlaces de Login y Register --}}
                    <div class="space-x-4">
                ...
                @auth
                    {{-- Si está autenticado, mostrar el dropdown de perfil --}}
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                                {{-- Solución al primer error: Auth::user() solo se accede aquí --}}
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        {{-- Enlace a la Home pública siempre visible en responsive --}}
        <div class="pt-2 pb-3 space-y-1">
            <x-responsive-nav-link :href="route('home')" :active="request()->routeIs('home')">
                {{ __('Home') }}
            </x-responsive-nav-link>
            @auth
                {{-- Enlace al Dashboard solo si está autenticado --}}
        ...
        @auth
            {{-- Solución al segundo error: Todo este bloque de usuario DEBE estar dentro de @auth --}}
        ...
        @else
            {{-- Mostrar Login/Register si es invitado en el menú responsive --}}
